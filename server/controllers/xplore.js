...
const getXploreBySearch = async (req, res) => {
  ...
  try {
    ...
    const xplores = await XPlore.find({
      $or: [{ title: { $regex: quer } }, { location: { $regex: quer } }],
    });
    res.json(xplores);
  } catch (err) {
    res.status(404).json({ message: "Something went wrong!" });
...
const likeXplore = async (req, res) => {
  ...
  try {
    ...
    const index = await xplore.likes.findIndex(
    ...
    const updatedXplore = await XPlore.findByIdAndUpdate(id, xplore, {
      new: true,
    });
