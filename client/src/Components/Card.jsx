      ...
      <MDBCard className={`h-100 mt-2 d-sm-flex ${Styles.card}`}>
        <div className="bg-image hover-zoom hover-overlay">
          ...
          <div
            ...
            onClick={() => {
              dispatch(updatefav(id));
            }}
          >
            {profile ? (
              <p>
                Click to {value ? "remove from " : "add to "} your favourites
                <img
                  src={value ? sad : fav}
                  alt="fav"
        ...
        <div className={Styles.creator}>
          Created By: {creator}{" "}
          {value ? (
            <img src={heart} className={Styles.fav} alt="heart" />
          ) : null}
        </div>
        <span className={`text-start tag-card ${Styles.tags}`}>
          <div>
            {tags.map((tag) => (
              <Link to={`/xplore/search/${tag}`} className={Styles.tag}>
                {" "}
                #{tag}
              </Link>
            ))}
        ...
        <MDBCardBody>
          ...
          <Link to={`/xplore/${_id}`} className={Styles.link}>
            Read More
          </Link>
        </MDBCardBody>
      </MDBCard>
